...
HNSpecial.settings.registerModule("infinite_scrolling", function () {
  ...
  function getButton(context) {
    return _.$("td.title > a[href^='news?p=']", context)[0];
  }
      ...
      if (loads === 1) {
        if (!notice) {
          var elem = _.createElement("div", {
            ...
            content:
              "Please keep scrolling if you want to access the search field and the footer. <span>(click to close)</span>",
          });
          elem.addEventListener("click", function () {
            this.classList.add("hnspecial-infinite-search-notice-hidden");
          });
          document.body.addEventListener("click", function (e) {
            if (
              !elem.classList.contains("hnspecial-infinite-search-notice-hidden") &&
              e.target !== elem
            ) {
  ...
  function loadLinks() {
    ...
    var url = "https://news.ycombinator.com/" + button.getAttribute("href");

    _.request(url, function (page) {
      var dummy = _.createElement("div");
      dummy.textContent = page;
